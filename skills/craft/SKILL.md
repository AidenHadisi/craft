...
Write `docs/plans/<feature>.md` yourself, mirroring the structure and depth of the worked example in [references/example-plan.md](references/example-plan.md). Build it incrementally — a full plan is large, so writing it in one edit tends to fail or truncate: first lay down the skeleton (the complete `## Architecture & design` section, then a one-line placeholder per step under `## Steps`, then `## Verification`), then flesh out one step per edit. Carry the architect's design in verbatim-ish — refine, don't compress; the design owns the *why and shape* while steps own the *what*, so don't restate step code up there. Each step gives its target file path and **complete literal code** a coder can reproduce with zero invention, or is a **manual action** the user runs (DDL, migration, install, env var) placed in order and marked manual so no coder is assigned it (see Step 1 of the example). Keep it well-formatted — file links, code blocks, tables, headers — like the example.
